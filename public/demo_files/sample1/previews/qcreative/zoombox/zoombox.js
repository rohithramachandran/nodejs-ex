                ...
                if(arg.length==0){
                    ...
                    setTimeout(function(){
                        ...
                        if(String(tempNr)!='NaN'){
                            ...
                            // console.warn(margs.from_deeplink);

                            var aux = '#'+margs.from_deeplink;

                            aux = aux.replace(/8767/g, '%');
                            aux = aux.replace('##','#');
                            ...
                            goto_item(auxobj, null, {
                                ...
                                console.info('zoombox error on open - ', err);
                            }
                        }
                    },1000)
...
//                console.info($.fn.vPlayer)
            ...
            console.log(' lets see data-biggallery' , _t, _t.attr('data-biggallery'),gallerymenu_arr.length, gallerymenu_arr);


            if(_t.attr('data-biggallery') && gallerymenu_arr.length==0){
        ...
        function gotoItem(arg, argtype, otherargs) {
            ...
            if(o.settings_deeplinking=='on' && has_history_api()==true && otherargs.forcenodeeplink!='on'){
                ...
                if(typeof($(otherargs.item).attr('id'))!='undefined'){
                    ...
                    console.info($(otherargs.item).attr('id'), encodeURIComponent($(otherargs.item).attr('id')))


                    var aux = encodeURIComponent($(otherargs.item).attr('id'));
            ...
            if (argtype == 'ajax') {
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        // console.info(_holder.children(), _holder.children().length);
                        ...
                        if(_holder.children().length>1){
                            ...
                            _theItem = _holder.children().eq(0);
                        }
...
//                    console.log(target_swiper, cthis, targetPositionX, up_x, down_x, sw_tw/5);
        ...
        function calculateDims(pargs){
            ...
            if(_holder){
                ...
                if(_holder.hasClass('type-ajax')){

                    // console.info(_theItem);

                    var _theItem_c = _theItem.children().eq(0);
                    if(_theItem_c && _theItem_c.hasClass && _theItem_c.hasClass('gallery-precon')){
                        // console.info("LUPII", iw,ih);


                        bigheight = _theItem_c.height();

                        if(just_resized_from_inner==false){
                            setTimeout(function(){
                            ...
                            setTimeout(function(){
                                ...
                                if(_theItem_c.find('.videogallery').eq(0).get(0) && _theItem_c.find('.videogallery').eq(0).get(0).api_handle_resize){

                                    _theItem_c.find('.videogallery').eq(0).get(0).api_handle_resize();
                                    _theItem_c.find('.videogallery').eq(0).get(0).api_handleResize_currVideo();
                                }
                            },2000);
                            setTimeout(function(){

                                if(_theItem_c.find('.videogallery').eq(0).get(0) && _theItem_c.find('.videogallery').eq(0).get(0).api_handleResize_currVideo){

                                    // console.info("HMM DADA");
                                    _theItem_c.find('.videogallery').eq(0).get(0).api_handleResize_currVideo();
                                }
                            },3000);

                            just_resized_from_inner=true;
